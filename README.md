...
현재 데스크톱 앱은 macOS Apple Silicon을 우선 지원하는 Tauri v2 기반 읽기 전용 내부 알파다. 사용자가 선택한 데이터 폴더를 기억하고 업무 항목·context·체크포인트 현황과 기본 구조 문제를 보여준다. Codex나 CLI가 파일을 바꾸면 화면을 자동으로 다시 검사한다.
...
업무 상세, 전체 JSON Schema 검증, 편집, 메뉴바와 알림은 다음 마일스톤에서 구현한다. 구체적인 범위와 데이터 무결성 전략은 [데스크톱 앱 구현 계획](./docs/desktop-app-plan.md)과 [Tauri 도입 결정](./docs/adr/0001-tauri-desktop-app.md)에 정리되어 있다.
