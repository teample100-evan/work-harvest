...
- 데스크톱 앱의 쓰기 편집, Finder 연결, 메뉴바와 알림
...
현재 데스크톱 앱은 macOS Apple Silicon을 우선 지원하는 Tauri v2 기반 읽기 전용 내부 알파다. 사용자가 선택한 데이터 폴더를 기억하고 업무 검색·상태 필터, 현재 context와 체크포인트 타임라인을 제공한다. Rust Core는 저장소의 공통 JSON Schema를 앱에 내장해 전체 문서와 파일 관계를 검사하며, Codex나 CLI가 파일을 바꾸면 화면을 자동으로 갱신한다.
...
체크포인트 전체 상세, Finder 연결, 오류 복구 안내는 현재 M1의 남은 범위이며 편집, 메뉴바와 알림은 이후 마일스톤에서 구현한다. 구체적인 범위와 데이터 무결성 전략은 [데스크톱 앱 구현 계획](./docs/desktop-app-plan.md)과 [Tauri 도입 결정](./docs/adr/0001-tauri-desktop-app.md)에 정리되어 있다.
