...
- 데스크톱 앱 개발 시 Rust stable과 macOS Xcode Command Line Tools
...
현재 데스크톱 앱은 macOS Apple Silicon을 우선 지원하는 Tauri v2 기반 읽기 전용 내부 알파다. 사용자가 선택한 데이터 폴더를 기억하고 업무 검색·상태 필터, 현재 context와 체크포인트 전체 상세를 제공한다. Finder에서 업무 위치를 확인하거나 Context·체크포인트 Markdown을 기본 앱으로 열 수 있다. Rust Core는 저장소의 공통 JSON Schema를 앱에 내장해 전체 문서와 파일 관계를 검사하며, Codex나 CLI가 파일을 바꾸면 화면을 자동으로 갱신한다.
...
읽기 전용 내부 알파인 M1은 완료됐다. 안전한 편집, 메뉴바 상시 실행과 알림은 이후 마일스톤에서 구현한다. 구체적인 범위와 데이터 무결성 전략은 [데스크톱 앱 구현 계획](./docs/desktop-app-plan.md)과 [Tauri 도입 결정](./docs/adr/0001-tauri-desktop-app.md)에 정리되어 있다.
