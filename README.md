...
M1과 M2 상시 실행 경험 1차 구현, 변경 업무 단위 증분 인덱스는 완료됐다. M2의 실제 24시간 watcher soak를 병행하면서 M3의 복구 가능한 쓰기 기반, Node writer 잠금 호환과 업무 항목·체크포인트·성과 노트 GUI 쓰기까지 완료했다. 다음 구현 단위는 M4 Rust CLI 통합이다. 구체적인 범위와 데이터 무결성 전략은 [데스크톱 앱 구현 계획](./docs/desktop-app-plan.md), [Tauri 도입 결정](./docs/adr/0001-tauri-desktop-app.md)과 [복구 가능한 쓰기 결정](./docs/adr/0002-recoverable-local-write-transactions.md)에 정리되어 있다.
