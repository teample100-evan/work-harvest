...
- 데스크톱 주간 성과보고서, 기간별 업무·Git·검증 결과 자동 집계와 저장 전 Markdown 검토
- Node와 같은 명령·JSON 계약을 제공하고 Rust Core를 직접 호출하는 native `wh` CLI
- Apple Silicon `wh`를 포함하는 macOS 앱·DMG와 태그 기반 서명·공증 릴리스 파이프라인
...
- `desktop:build`: native `wh`를 포함한 `.app`과 DMG 생성
- `desktop:bundle:verify`: 앱 안의 `wh` 실행·예제 데이터 검증과 서명 정보 확인
- `check:all`: 기존 Node CLI, React 프론트엔드와 Rust workspace 전체 검증

M1과 M2 상시 실행 경험 1차 구현, 변경 업무 단위 증분 인덱스는 완료됐다. M2의 실제 24시간 watcher soak를 병행하면서 M3 안전 쓰기와 M4 native Rust CLI 통합까지 완료했다. M5는 앱에 CLI를 동봉하고 Developer ID 서명·공증 후 앱과 standalone CLI를 같은 GitHub 초안 Release에 올리는 파이프라인까지 구현했다. 첫 서명 릴리스와 자동 업데이트 키 설정, 실사용 안정화 뒤 Node fallback 제거가 남아 있다. 자세한 배포 절차는 [macOS 배포](./docs/distribution.md), 구체적인 범위와 데이터 무결성 전략은 [데스크톱 앱 구현 계획](./docs/desktop-app-plan.md)에 정리되어 있다.
...
표준 위치가 아닌 앱 번들을 직접 지정할 수도 있다.

```bash
export WORK_HARVEST_APP_PATH="$HOME/Applications/Work Harvest.app"
```

`WORK_HARVEST_HOME`은 기록 데이터 위치, `WORK_HARVEST_CLI_HOME`은 CLI checkout 위치, `WORK_HARVEST_CLI_BIN`은 실행할 native CLI 위치, `WORK_HARVEST_APP_PATH`는 bundled CLI를 찾을 앱 위치다. Skill 래퍼는 명시한 CLI 또는 앱, checkout release, 표준 위치에 설치한 앱, `~/.local/bin`, checkout debug 순으로 탐색한다.
